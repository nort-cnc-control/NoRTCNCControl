        ...
        public ModbusToolCommand CreateSpindleToolCommand(SpindleRotationState rotation, double speed)
        {
            ...
            switch (rotation)
            {
                case SpindleRotationState.Off:
                    registers[0] = new ModbusRegister { RegisterId = runRegister, RegisterValue = runNone };
                    delay = 0;
                    break;
                case SpindleRotationState.Clockwise:
                    registers[0] = new ModbusRegister { RegisterId = runRegister, RegisterValue = runForward };
                    delay = 3000;
                    break;
                case SpindleRotationState.CounterClockwise:
                    registers[0] = new ModbusRegister { RegisterId = runRegister, RegisterValue = runReverse };
                    delay = 3000;
                    break;
            }
            registers[1] = new ModbusRegister { RegisterId = speedRegister, RegisterValue = (UInt16)(speed / 60.0 * 100) };
